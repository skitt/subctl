...
func buildBenchmarkRunner(run func(intraCluster, verbose bool) error) func(command *cobra.Command, args []string) {
	return func(command *cobra.Command, args []string) {
		...
		// - kubecontext(s)
		if benchmarkRestConfigProducer.CountSelectedContexts() > 0 {
			exit.OnError(benchmarkRestConfigProducer.RunOnSelectedContexts(
		...
		exit.OnError(benchmarkRestConfigProducer.RunOnSelectedContext(
			func(fromClusterInfo *cluster.Info, _ string, status reporter.Interface) error {
				if benchmarkRestConfigProducer.HasSelectedPrefixedContext("to") {
					return benchmarkRestConfigProducer.RunOnSelectedPrefixedContext( // nolint:wrapcheck // No need to wrap errors here.
						"to",
