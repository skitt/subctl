...
// NewProducer creates a producer which is expected to take its configuration from flags.
// SetupFlags() must be called before attempting to use the producer.
func NewProducer() *Producer {
	return &Producer{}
}

// NewProducerFrom creates a producer using the given kubeconfig file and kubecontext (if any).
// The resulting producer can be used immediately and must not be set up with flags.
...
// SetupFlags sets the given flags up to control the clientcmd configuration(s).
...
func (rcp *Producer) setupContextFlags(loadingRules *clientcmd.ClientConfigLoadingRules, flags *pflag.FlagSet, prefix string) *configAndOverrides {
...
func (rcp *Producer) RunOnSelectedPrefixedContext(prefix string, function PerContextFn, status reporter.Interface) error {
	...
	if ok {
		...
		namespace, _, err := clientConfig.config.Namespace()
		if err != nil {
